...
class LLaVATrainer(Trainer):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, "tune_mm_mlp_adapter", False) or (
            hasattr(self.args, "mm_tunable_parts") and (len(self.args.mm_tunable_parts.split(",")) == 3 and ("mm_mlp_adapter" in self.args.mm_tunable_parts or "mm_change_detector" in self.args.mm_tunable_parts or "mm_seg_head" in self.args.mm_tunable_parts))
        ):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.epoch > 1:
            ...
            w_i = torch.Tensor(self.train_loss_buffer[:, 1] / (self.train_loss_buffer[:, 0] + epsilon)).to(self.args.device)
        ...
        self.train_loss_buffer[:, 1] = torch.Tensor([loss.item() for loss in losses]).to(self.args.device)  # 更新当前损失
