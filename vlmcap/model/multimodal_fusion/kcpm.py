...
class KCPM(nn.Module):
    ...
    def __init__(self,
        ...
        self.flow_make_g = nn.Sequential(
            nn.Conv2d(in_channels*2, in_channels*2, kernel_size=kernel_size_g, padding=(kernel_size_g-1)//2, bias=False, groups=in_channels*2),
        ...
        self.flows = nn.ModuleList([
            Flow_Block(in_channels=self.in_channels, kernel_size=kernel_size_l)
            for _ in range(4)
        ])
    ...
    def forward(self, x1, x2, fusion_policy=None):
        ...
        output_g = torch.cat([x1, x2], dim=1)
        flow_g = self.flow_make_g(output_g)
        f1_g, f2_g = torch.chunk(flow_g, 2, dim=1)
        x1_feat_g = self.warp(x1, f1_g) - x2
        x2_feat_g = self.warp(x2, f2_g) - x1
        ...
        for flow_block, out, x1_c, x2_c in zip(
            ...
            x1_feats_l.append(x1_feat)
            x2_feats_l.append(x2_feat)

        x1_feat_l = torch.cat(x1_feats_l, dim=1)
        x2_feat_l = torch.cat(x2_feats_l, dim=1)

        x1_feat = x1_feat_g + x1_feat_l
        x2_feat = x2_feat_g + x2_feat_l
        
        if fusion_policy is None:
            return x1_feat, x2_feat
        
        output = self.fusion(x1_feat, x2_feat, fusion_policy)
