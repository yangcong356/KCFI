    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None):
        ...
        if type(images) is list or images.ndim == 5:
            if type(images) is list:
                images = [x.unsqueeze(0) if x.ndim == 3 else x for x in images] # [C, H, W] > [1, C, H, W]
            ...
            concat_images = torch.cat([image for image in images_list], dim=0)  #   [2B, C, H, W]
            split_sizes = [image.shape[0] for image in images_list] #   [B, B]
            if self.config.mm_fusion_policy is not None:
